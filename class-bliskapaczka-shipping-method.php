...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ), true ) ) {
	...
	function bliskapaczka_map_shipping_method() {
		include_once 'includes/class-bliskapaczka-map-shipping-method.php';
	}
	...
	function bliskapaczka_courier_shipping_method() {
		include_once 'includes/class-bliskapaczka-courier-shipping-method.php';
	}
	...
	function bliskapaczka_delivery_to_point_front_view( $method ) {
		...
		if ( Bliskapaczka_Map_Shipping_Method::get_identity() === $method->id && is_checkout() === true ) {
			...
			$chosen_shipping_method  = is_array( $chosen_shipping_methods ) && isset( $chosen_shipping_methods[0] ) ? $chosen_shipping_methods[0] : null;
	...
	function bliskapaczka_delivery_to_point_print_widget_in_checkout_view( $with_price = true ) {
        ...
        echo " <a class='bliskapaczka-delivery-to-point-btn' href='#bpWidget_wrapper' " .
		 ...
		 echo '<input name="bliskapaczka_point_code" type="hidden" class="bliskapaczka-point-code" value="' . esc_html( $pos_code ) . '" />';
		 echo '<input name="bliskapaczka_point_operator" type="hidden" class="bliskapaczka-point-operator" value="' . esc_html( $pos_operator ) . '" />';
	...
	function bliskapaczka_delivery_to_point_print_choosed_point_address( $with_price = true ) {
		...
		echo '<div class="bpWidget_aboutPoint" style="width: 100%; ' . ( ( ! isset( $pos_info ) ) ? ' display: none; ' : '' ) . '">';
		echo '<p>' . esc_html( __( 'Selected Point', 'bliskapaczka-pl' ) ) . ': <span class="bpWidget_aboutPoint_posData">';
		if ( isset( $pos_info ) ) {
            // @codingStandardsIgnoreStart
...
// 			return $packages;
	...
	function bliskapaczka_create_order_via_api( $order_id ) {
		...
		if (!isset($operator)) {
			throw new \Exception(esc_html(__( 'Przeładuj stronę i wybierz ponownie kuriera.', 'bliskapaczka-pl' )));
		}
	 ...
	 * Include JS.
	 */
	function bliskapaczka_add_styles_and_scripts() {
		...
		wp_register_style( 'widget-styles-bliskapaczka', plugin_dir_url( __FILE__ ) . 'assets/css/bliskapaczka.css', array(), 'v11', false );
		wp_enqueue_style( 'widget-styles-bliskapaczka' );

		wp_register_script( 'plugin-script', plugin_dir_url( __FILE__ ) . 'assets/js/bliskapaczka.js', array(), 'v11', false );
	...
	add_action( 'wp_enqueue_scripts', 'bliskapaczka_add_styles_and_scripts' );
	...
	function bliskapaczka_checkout_validation( $fields, $errors ) {
		...
		$bliskapaczka_method_checker = null;

		$chosen_shipping_methods = WC()->session->get( 'chosen_shipping_methods' );

		if ( is_array( $chosen_shipping_methods ) && isset( $chosen_shipping_methods[0] ) ) {
			$bliskapaczka_method_checker = $chosen_shipping_methods[0];
		}
	...
	function bliskapaczka_disabled_checkout_button( $button_html ) {
        ...
        /* Get id of chosen Shipping method */
        $method_id = wc_get_chosen_shipping_method_ids()[0];
        $is_flexible = Bliskapaczka_Flexible_Shipping_Integration::isFlexibleShippingMethod( $method_id );
        ...
        foreach ( $packages as $key => $package ) {
            foreach($package['rates'] as $rate_id => $rate ){
                $available_shipping_methods[] =  $rate;
            }
        }

        /* Search for chosen shipping method */
        $check_for_method = array_search( $chosen_shipping_methods[0], array_column($available_shipping_methods, 'id'));
        $meta = $available_shipping_methods[$check_for_method]->get_meta_data();
        $meta_fs = $meta['_fs_method'];
        $meta_fs_operator = $meta['_fs_method']['bliskapaczka_fs_operator'];

        if ( is_checkout() && isset( $meta_fs['method_integration'] ) && Bliskapaczka_Flexible_Shipping_Integration::INTEGRATION_NAME === $meta_fs['method_integration']  && empty ( $point_operator ) ) {
            $disabled = true;
        }
        }
	...
	function bliskapaczka_calculated_total( $total, $cart ) {
		...
		$chosen_method     = is_array( $chosen_methods ) && isset( $chosen_methods[0] ) ? $chosen_methods[0] : null;
	...
	function bliskapaczka_get_price() {
		...
		$chosen_methods = WC()->session->get( 'chosen_shipping_methods' );
		$chosen_method  = is_array( $chosen_methods ) && isset( $chosen_methods ) ? $chosen_methods[0] : null;
		...
		switch ( $chosen_method ) {
			...
			case Bliskapaczka_Map_Shipping_Method::get_identity():
				...
				/* Get price list for method and payment type */
                $price_list = $bliskapaczka->get_price_list( WC()->cart->get_cart_contents_total(), $is_cod );
                ...
                if (count($operators) > 0) {
	                $prices = array_column($operators, 'price');
	                $index = array_search(min($prices), $prices, true);
	                $best_price_operator = $operators[$index];
                }
		...
		if ( isset( $bliskapaczka ) && isset( $operator ) ) {
			...
			} catch (Bliskapaczka_Exception $e) {
				WC()->session->__unset( 'bliskapaczka_door_operator' );
				WC()->session->__unset( 'bliskapaczka_point_operator' );
				WC()->session->__unset( 'bliskapaczka_point_code' );
				
			}
		}
        /* If map method is selected and map operator is not chosen return price of cheapest available operator */
		if ( isset ($bliskapaczka) && !isset ( $operator ) && isset($best_price_operator) && Bliskapaczka_Map_Shipping_Method::get_identity() === $chosen_method ) {
        	return $best_price_operator['price'];
        }
	...
	function bliskapaczka_checkout_update_order_review( $data ) {
		...
		if ( isset( $post_data['shipping_method'] ) && is_array( $post_data['shipping_method'] ) ) {
			...
			$chosen_shipping_methods = WC()->session->get( 'chosen_shipping_methods' );
			$chosen_method           = is_array( $chosen_shipping_methods ) && isset( $chosen_shipping_methods ) ? $chosen_shipping_methods[0] : null;
		}
		...
		if ( is_object($price_list) && count($price_list) > 0 ) {
			...
			if ( isset($price_list[0]) && $price_list[0] instanceof Bliskapaczka_Price_List_Item ) {
				WC()->session->set( $operator_index, $price_list[0]->operator() );
			} else {
				WC()->session->__unset( $operator_index );
			}
	...
	/**
	 * Display point address in shipping data in WooCommerce order.
	 * 
	 * This method are changing display in admin, front and mail.
	 */
	function bliskapaczka_order_get_formatted_shipping_address($address, $raw_address, $order) 
	{
		$helper = Bliskapaczka_Shipping_Method_Helper::instance();
		$shipping_method_id = $helper->getWCShipingMethodId( $order );
		
		if ( false === in_array($shipping_method_id, [Bliskapaczka_Courier_Shipping_Method::get_identity(), Bliskapaczka_Map_Shipping_Method::get_identity()])) {
			return $address;
		}
		...
		if ( null !== $point_code) {
			$delivery_info .= '<div class="bliskapaczka-order-shipping_address">';
			$delivery_info .= '<br><br>' . __('<strong>Punkt odbioru:</strong>', 'bliskapaczka_pl');
			$delivery_info .= '<br>' . $point_code;
			$delivery_info .= '<br>' . preg_replace('|(\d\d-\d\d\d)|', '<br>$1', $point_address, 1);
			$delivery_info .= '</div>';
		}
	...
	add_filter('woocommerce_order_get_formatted_shipping_address', 'bliskapaczka_order_get_formatted_shipping_address', 10, 3);
