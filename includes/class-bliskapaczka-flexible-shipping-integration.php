...
class Bliskapaczka_Flexible_Shipping_Integration {
	...
	public function fs_add_method( $add_method, $shipping_method, $package ) {
		...
		if ( isset( $shipping_method['method_integration'] )
			...
			// For map widget, we don't verify availability.
	...
	public function wc_after_shipping_rate( WC_Shipping_Rate $method, $index ) {
		...
		if ( true === self::isFlexibleShippingMethod($method->method_id ) ) {
	...
	public function send_order_to_api( $order_id ) {
		...
		if ( false === self::isFlexibleShippingMethod($shipping_method_id) ) {
			return;
		}
	...
	/**
	 * Verify if given shipping method identity is from Flexbile Shipping
	 *  
	 * @param string $shipping_method_id Shipping method id to verify 
	 * @return boolean
	 */
	public static function isFlexibleShippingMethod($shipping_method_id) 
	{
		return in_array($shipping_method_id, ['flexible_shipping', 'flexible_shipping_single'], true);
	}
