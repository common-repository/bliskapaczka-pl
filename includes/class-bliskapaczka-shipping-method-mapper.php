...
class Bliskapaczka_Shipping_Method_Mapper {
	...
	protected function _prepareDestinationData( $data, WC_Order $order ) {
		...
		} elseif ( $pattern_group_counter == 2 ) {
		...
		$data['receiverCountryCode']    = $shippingAddress['country'];
		return $data;
	}
	...
	public function prepareInsuranceDataIfNeeded( $data, WC_Order $order ) {
		...
		if (in_array($operatorName, ['FEDEX', 'DPD'])) {
			$insuranceValue = $order->get_total();
		} else if ($deliveryType === 'D2D') {
			
			if (true  === $isCod) {
				switch ($operatorName) {
					case 'INPOST':
						// insurace for inpost muast be grather than value
						$insuranceValue = $order->get_total() + 1;
						break;
					case 'DHL':
						$insuranceValue = $order->get_total();
						break;
			...
			if ($insuranceValue > 0) {
				$data['parcel']['insuranceValue'] = $insuranceValue;
			}
		}
	...
	protected function _prepareSenderData( $data, Bliskapaczka_Shipping_Method_Helper $helper, $settings ) {
        if ( $settings[ $helper::SENDER_COMPANY ] ) {
            $data['senderCompanyName'] = $settings[ $helper::SENDER_COMPANY ];
        }
