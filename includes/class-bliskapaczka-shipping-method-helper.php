...
class Bliskapaczka_Shipping_Method_Helper {
	...
	const SENDER_COMPANY         = 'BLISKAPACZKA_SENDER_COMPANY';
	...
	/**
	 * Returns a name of shop engine
	 */
	public static function shopReportedEngineName() {
		return 'woocommerce';
	}
	...
	public function getApiClientPricing() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Pricing(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	public function getApiClientPos() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Pos(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	public function getApiClientOrder() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Order(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	public function getApiClientOrderAdvice() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Order\Advice(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	public function getApiClientTodoorAdvice() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Todoor\Advice(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	public function getApiClientPickup() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Order\Pickup(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	public function getApiClientAvailableServices() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Order\AvailableServices(
				$this->getApiKey(),
				$this->getApiMode()
			)
		);
	...
	/**
	 * 
	 * @return \Bliskapaczka\ApiClient\AbstractBliskapaczka\Report
	 */
	public function getApiClientReportPickup() {
		return $this->decorate_bp_api_request(
			new \Bliskapaczka\ApiClient\Bliskapaczka\Report(
				$this->getApiKey(),
				$this->getApiMode()
				)
			);
	...
	private function decorate_bp_api_request( AbstractBliskapaczka $client ) {
		...
		$client
			->setShopName( self::shopReportedEngineName() )
	...
	public function send_order_to_api( WC_Order $order, $delivery_to_point ) {
		...
		try {
			...
			if ( $this->isAutoAdvice() === true ) {
				...
				$order->save();
//				Actuallly the auto pickup flag is working correctly 
// 				so we don't need to do this manualy 
// 				$this->request_auto_pickup( $order );
	...
	public function request_auto_pickup( WC_Order $order ) {
		...
		try {
			   $pickup_params       = $mapper->prepareDataForPickup(
				   $bliskapaczka,
				   [
			...
			if ( isset( $api_pickup_response['id'] ) && isset( $api_pickup_response['number'] ) ) {
				  ...
				  $logger->error(
					  'BLISKAPACZKA: Order pickup faild: Undefined resposne from API',
					  [
						  'order_id'               => $order->id,
	...
	/**
	 * Get order from bliskapaczka api
	 * 
	 * @param string $order_number Order number
	 * @return NULL|mixed
	 */
	public function apiOrderGet( $order_number )
	{
		$client = $this->getApiClientOrder();
		
		$client->setOrderId( $order_number );
	...
	/**
	 * Get contnet of pickup report for given numbers
	 * 
	 * @param string $ordersNumbers BliskaPaczka orders numbers, delimented by ","
	 * @return string|null PDF report content or null if not found. 
	 */
	public function apiPickupReportGet( $ordersNumbers )
	{
		$client = $this->getApiClientReportPickup();
	
		$client->setNumbers($ordersNumbers);
